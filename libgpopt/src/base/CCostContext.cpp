	...
	const
{
	...
	// this should be the top most rule as we want to override cost based check in case the
	// trace flag is set.
	if (GPOS_FTRACE(EopttraceForceThreeStageScalarDQA))
	{
		if (CUtils::FPhysicalAgg(Pgexpr()->Pop()) && CUtils::FPhysicalAgg(pcc->Pgexpr()->Pop()))
		{
			...
			// we want to favor 3-stage agg over 2-stage scalar DQA agg. So here we mark 2-stage scalar DQA as less optimal.
...
BOOL
CCostContext::IsTwoStageScalarDQA
		(
				const CCostContext *pcc
		)
	const
