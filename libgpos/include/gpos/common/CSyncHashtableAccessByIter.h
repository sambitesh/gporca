...
namespace gpos
{
	...
	private:
		...
		// no copy ctor
		CSyncHashtableAccessByIter<T, K, S>(const CSyncHashtableAccessByIter<T, K, S> &);
		...
		FirstValid(T *value) const
		{
			...
			T *curr = value;
			while (NULL != curr && !Base::GetHashTable().IsValid(Base::GetHashTable().Key(curr)))
			{
				curr = Base::Next(curr);
			}
	...
	public:
		// ctor
		explicit CSyncHashtableAccessByIter<T, K, S>(CSyncHashtableIter<T, K, S> &iter)
		...
		Value() const
		{
			GPOS_ASSERT(m_iter.m_invalid_elem_inserted && "Iterator's advance is not called");
