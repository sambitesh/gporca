...
void *
CCustomAllocator::fnAlloc(SIZE_T ulAlloc)
{
...
void
CCustomAllocator::fnFree(void *pMem)
{
...
BOOL
gpos::FUseCustomAllocator(CMainArgs *pma)
{
	CHAR ch = '\0';
...
CMemoryPoolAllocTest::EresUnittest()
{
	CUnittest rgut[] = {GPOS_UNITTEST_FUNC(CMemoryPoolAllocTest::EresUnittest_AllocFree)};
...
CMemoryPoolAllocTest::EresUnittest_AllocFree()
{
	...
	GPOS_RTL_ASSERT((ulTotalMemoryAllocated + 100) ==
					CCustomAllocator::UlGetTotalMemoryAllocation());
	customAlloc.Free(pMem);
	GPOS_RTL_ASSERT(ulTotalMemoryAllocated == CCustomAllocator::UlGetTotalMemoryAllocation());
	return GPOS_OK;
}
