...
//
//---------------------------------------------------------------------------
CWorker::CWorker(ULONG thread_id, ULONG stack_size, ULONG_PTR stack_start)
	: m_task(NULL), m_thread_id(thread_id), m_stack_size(stack_size), m_stack_start(stack_start)
{
#ifdef GPOS_DEBUG
...
CWorker::Run()
{
	...
	while (CWorkerPoolManager::EsrExecTask ==
		   CWorkerPoolManager::WorkerPoolManager()->RespondToNextTaskRequest(&task))
	{
		Execute(task);
	}
}
...
CWorker::CheckTimeSlice()
{
	if (IWorker::m_enforce_time_slices)
	{
		...
		ULONG threads =
			std::max((ULONG) 1, CWorkerPoolManager::WorkerPoolManager()->GetNumWorkersRunning());
		...
		if (GPOS_CHECK_ABORT_MAX_INTERVAL_MSEC * threads <= interval && m_task->IsRunning())
		{
			...
			ResetTimeSlice();
			GPOS_RAISE(
				CException::ExmaSystem, CException::ExmiAbortTimeout, interval, str.GetBuffer());
		}
	}
}
...
//---------------------------------------------------------------------------
BOOL
CWorker::CanAcquireSpinlock(const CSpinlockBase *slock) const
{
...
//---------------------------------------------------------------------------
void
CWorker::RegisterSpinlock(CSpinlockBase *slock)
{
...
//---------------------------------------------------------------------------
void
CWorker::UnregisterSpinlock(CSpinlockBase *slock)
{
...
//---------------------------------------------------------------------------
void
CWorker::RegisterMutex(CMutexBase *mutex)
{
...
//---------------------------------------------------------------------------
void
CWorker::UnregisterMutex(CMutexBase *mutex)
{
	GPOS_ASSERT(NULL != mutex);
	GPOS_ASSERT(GPOS_OK == m_mutex_list.Find(mutex));
